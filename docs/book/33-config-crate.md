...
chapter: 33
session_id: M15-1
title: Config crate and global config
phase: D
estimated_time_minutes: 90
artifact: Config loads from ~/.config/lazydap/
status: stub
related_milestone: docs/implementation/tasks/M15-config-file.md
---

# Chapter 33 — Config crate and global config
...
> Session ID: `M15-1` · Phase D · ~90 min · [Underlying milestone](../implementation/tasks/M15-config-file.md)

## What you'll learn

crates/config/src/lib.rs. Read ~/.config/lazydap/config.toml. XDG paths. Defaults. How config-rs-style merging works, why we don't use the config crate (one less dep), env var overrides.

## What you'll build

Config loads from ~/.config/lazydap/
...
The companion teaching-notes file at `docs/teaching/notes/33-config-crate.md` will be created during the live session.
