...
fn continuing_a_program_that_is_already_running_reaches_the_breakpoint() {
    ...
    // (quirk 18), so against `exits` the resume ran a program that had already
    ...
    sandbox.breakpoint("spins.go", 16);
    let launched = sandbox.json(&[
        "--format",
        "json",
        "launch",
        &fixture("spins.go").to_string_lossy(),
    ]);
    assert_eq!(launched["breakpoints"][0]["line"], 16, "got: {launched}");
    assert_eq!(
    ...
    assert_eq!(blob["state"], "paused", "got: {blob}");
    assert_eq!(blob["reason"], "breakpoint", "got: {blob}");
    assert_eq!(blob["frame"]["line"], 16, "got: {blob}");

    // `spins.go` never ends on its own, so it is still sitting at the
    // breakpoint. `Sandbox::drop` disconnects, which terminates it, and then
    // checks for strays — the same teardown every other test here relies on.
}
