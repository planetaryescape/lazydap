---
title: "lazydap step"
description: "Run the next line, stepping over any call in it"
---

:::note[Generated page]
From `lazydap step --help`. To change it, change the clap definition in [`crates/daemon/src/cli.rs`](https://github.com/planetaryescape/lazydap/blob/main/crates/daemon/src/cli.rs), then run `npm run generate` in `site/` and commit the result. CI fails if this page and the binary disagree.
:::
