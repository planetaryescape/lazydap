---
title: "lazydap scopes"
description: "Show the variable scopes of a frame"
---

:::note[Generated page]
From `lazydap scopes --help`. To change it, change the clap definition in [`crates/daemon/src/cli.rs`](https://github.com/planetaryescape/lazydap/blob/main/crates/daemon/src/cli.rs), then run `npm run generate` in `site/` and commit the result. CI fails if this page and the binary disagree.
:::

Show the variable scopes of a frame

```text
lazydap scopes [OPTIONS]
```

## Options

| Flag | Description |
| --- | --- |
| `--frame <FRAME>` | A frame id from `lazydap stack` — not a position in it. Defaults to the top frame |

Plus the [options every command takes](/reference/cli/#options-every-command-takes).

## See also

- [CLI overview](/reference/cli/) — every command in one list
- [JSON output](/reference/json-output/) — the shape of what comes back
- [Errors and exit codes](/reference/errors/) — what to do when it fails
